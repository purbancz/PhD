...
samo w~sobie wydaje się być twierdzeniem wewnętrznie sprzecznym i~naruszającym prawo tożsamości. Ratunku dla takiego
sformułowania można szukać na przykład w~uznaniu, że słowo ,,Bóg'' występuje w~innym
znaczeniu w podmiocie i~orzeczniku tego zdania lub -- co 
wykorzystuje Rojek w~swojej interpretacji teologii negatywnej -- negacja użyta w tym zdaniu nie jest negacją
klasyczną. W~końcu Areopagita konstatuje, że o~Bogu nie możemy nic
...
Teza \eqref{rojek-T4} mówi o niepoznawalności Boga, przy czym Rojek dopuszcza także rozumienie jej jako tezy o niewyrażalności boskiej istoty. W pismach Dionizego rozróżnienie między niepoznawalnością i niewyrażalnością jest zatarte. Teza ta nie tylko zdaje się podważać zasadność wysuwania tez \eqref{rojek-T1}-\eqref{rojek-T3}, lecz także zasadność utrzymywania samej siebie.
...
Ponadto jednoczesne utrzymywanie pełnego zbioru zdań teologii negatywnej, do którego
Rojek zalicza tezy \eqref{rojek-T1}-\eqref{rojek-T4}
wyabstrahowane z \textit{Teologii mistycznej}
Pseudo-Dionizego Areopagity, dodatkowo mnoży logiczne problemy i otwiera drogę do kolejnych niespójności
...
uważać za odrębną interpretacje teologii negatywnej. Umieszczenie tego fragmentu analiz Rojka w niniejszym rozdziale posiada jednak dodatkowe powody.
...
W końcu, do tego samego wniosku prowadzą nas Rojka komentarze do modelu, który sam zaproponował -- według niego niepoznawalność Boga polega na niemożliwości \textit{twierdzenia}, że posiada on pozytywne własności.
...
\begin{flalign*}
&\forall \mathcal{L}\ N_w(g,\mathcal{L}).&\tag{MNT}
\end{flalign*}

Rojek wydaje się być usatysfakcjonowany ustaleniami Bocheńskiego i przekonany, że ujęcie teologii milczenia w postaci \ref{sil-boch-MNT} ratuje tę teorię przed sprzecznościami\footnote{Jak pokazałem w poprzednim rozdziale (rozdz. \ref{sil-boch-dyskusja}), można mieć co do tego wątpliwości.}.
Powyższą formułę wraz dodatkowymi uwagami głoszącymi, że mamy tu do czynienia z wypowiedzią metajęzykową, nazywa ,,modelem'' teorii Niewysławialnego. Przyjmuje też w pełni Bocheńskiego argumenty przeciwko teorii Niewysławialnego -- fakt, że nie możemy przypisać Bogu żadnej własności przedmiotowo-językowej sprawia, że jest ona niezgodna z faktycznym dyskursem religijnym i religijną \textit{praxis}, a~więc dyskwalifikuje ją jako poprawną teorię Boga. Ze względu na jej uwikłanie w~swoje \textit{zewnętrzne} paradoksy należy ją odrzucić.
...
Rojek dodaje do tego argument, umieszczony w kontekście
swoich analiz. Mianowicie, jeśli teoria Niewysławialnego ma być
właściwą interpretacją teologii negatywnej, należy poddać w wątpliwość
zasadność jednoczesnego utrzymywania tez \eqref{rojek-T1}-\eqref{rojek-T3}. Jeśli Bóg jest
niewyrażalny, cały język religijny jest pozbawiony znaczenia, nie
możemy więc ani sensownie potwierdzić, ani zaprzeczyć żadnym z Jego
...
Dla Rojka jest to wskazówka, by nie interpretować jej
semantycznie, lecz epistemologicznie lub nawet
ontologicznie\footnote{Zob. P. Rojek, dz. cyt., s. 223.}. W ten sposób przechodzi od
teorii Niewysławialnego do agnostycznej teorii Niepoznawalnego.
...
błędnie. W ogóle w przedstawieniu teorii Niepoznawalnego w szacie logiki nieokreśloności Zinowjewa odnaleźć można dużo niezgrabności. Por. tamże. Kładę to jednak na karb nietypowej notacji stosowanej przez radzieckiego logika, którą Rojek próbuje przeredagować według bardziej współczesnych standardów.} staje się dwuznaczne. Może ono przyjąć jedno
...
$\neg Q(x)$\footnote{Rojek używa tych symboli na odwrót, to jest $\neg$ stosuje na oznaczenie negacji wewnętrznej a $sim$ używa do oddania negacji zewnętrznej. Dodatkowo formułę negowaną za pomocą
...
formułach \eqref{rojek-T2prim} i \eqref{rojek-T3prim}.
\begin{flalign}
&    G(x) \to  (\forall Q (P(Q) \to  \neg  Q(x) \land
\neg {\sim} Q(x)). &
\end{flalign}
...
rekonstrukcję tez \eqref{rojek-T1} oraz \eqref{rojek-T3}. Ponadto, jak Rojek podaje za Bocheńskim, nie da się go pogodzić z
...
rozdział I, 2. }. Uznawał on więc możliwość jednoczesnego utrzymywania
...
zauważa Rojek, w podanych przykładach przeczenie ma również pozytywny, nie tylko negatywny
...
teologie agnostyczne, jak i negatywna teologia negatywna nie są zgodne z
...
formalizacja stanowi dla Rojka rzekomo spójny model dla wszystkich czterech tez doktryny
Psudo-Dionizego. Ponadto, jej treść jest zbliżona zdaje się być zbliżona teorii analogii św. Tomasza z Akwinu. Według Rojka, za jej
